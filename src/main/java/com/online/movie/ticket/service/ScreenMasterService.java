...
import com.online.movie.ticket.repository.ScreenMasterRepository;
import com.online.movie.ticket.validation.ScreenMasterValidtor;
...
public class ScreenMasterService {
	...
	@Autowired
	ScreenMasterValidtor screenMasterValidtor;
	...
	public BaseDto get(Long id) {
		...
		try {
			...
			ValidateUtil.notNull(id, ErrorCode.SCREEN_ID_NOT_NULL);
			List<Long> results = new ArrayList<Long>();
			results.add(id);
			List <ScreenMaster> screenMaster = screenMasterRepository.findAllById(results);
		
			ValidateUtil.notNull(screenMaster, ErrorCode.SCREEN_ID_NOT_NULL);

			log.info("Successfully getting State by id.....[" +results+ "]");

			baseDto.setResponse(ErrorCode.SUCCESS);
			if (screenMaster.size() >0)
			baseDto.setResponseObject(screenMaster.get(0));
			else
	...
	public BaseDto create(ScreenMaster screenMaster) {
		log.debug("Create method is called.....");

		BaseDto baseDto = new BaseDto();
		try {
			
			screenMasterValidtor.validate(screenMaster);
			
			// Setting track time & user...
			screenMaster.setSystemTrack(TimeUtil.getCreateSystemTrack());
			// Saving 
			ScreenMaster screenMasterCreated = screenMasterRepository.save(screenMaster);
		
			log.debug("Screen Master Saved successfully.....[" + screenMasterCreated.getId() + "]");

			baseDto.setResponseCode(ErrorCode.SUCCESS.getCode() );
			baseDto.setResponseObject(screenMasterCreated);
			...
			if (exceptionCause1.contains("UK_SCREEN_NAME")) {
				baseDto.setResponse(ErrorCode.SCREEN_NAME_ALREADY_EXIST);
			}
		}
		return baseDto;
	}
	
	public BaseDto update(ScreenMaster existingScreenMaster) {

		log.debug("update method is called....ID [ " + existingScreenMaster.getId() + "]");

		BaseDto baseDto = new BaseDto();

		try {

			screenMasterValidtor.validate(existingScreenMaster);

			
		
			List<Long> id = new ArrayList<Long>();
			id.add(existingScreenMaster.getId());
			List <ScreenMaster> screenMaster = screenMasterRepository.findAllById(id);
			
			ValidateUtil.notNull(screenMaster, ErrorCode.SCREEN_NOT_FOUND);
			...
			if (screenMaster.size() >0)
			{
				existingScreenMaster.setSystemTrack(screenMaster.get(0).getSystemTrack());
				TimeUtil.setUpdateSystemTrack(existingScreenMaster.getSystemTrack());
			}
			...
			existingScreenMaster = screenMasterRepository.save(existingScreenMaster);
			
			log.debug("Screen Master Updated successfully.....[" + existingScreenMaster.getId() + "]");

			baseDto.setResponse(ErrorCode.SUCCESS);

			baseDto.setResponseObject(existingScreenMaster);
	...
	public BaseDto delete(Long id) {
		...
		try {
			...
			log.info("Screen Master Deleted Successfully....[" + id + "]");
	...
	public BaseDto getAll() {
		...
		try {
			...
			baseDto.setResponse(ErrorCode.SUCCESS);
			baseDto.setResponseObject(screenMasterList);
